      ...
      <main className="pt-32 pb-24 bg-gray-50">
        ...
        <section className="mt-20 max-w-7xl mx-auto px-6">
          <div className="grid md:grid-cols-2 gap-12 items-center bg-white rounded-3xl p-8 shadow">
            ...
            <div className="relative h-80 rounded-2xl overflow-hidden">
              {/* Skeleton */}
              {!loaded['featured'] && <div className="absolute inset-0 bg-gray-200 animate-pulse z-10" />}
              ...
              <img
                ...
                alt="Featured Blog"
                loading="lazy"
                onLoad={() => setLoaded(prev => ({ ...prev, featured: true }))}
                className={`relative z-20 h-full w-full object-cover rounded-2xl transition-all duration-700
                  ${loaded['featured'] ? "opacity-100 blur-0 scale-100" : "opacity-0 blur-lg scale-105"}
                `}
              />
            </div>
        ...
        <section className="mt-20 max-w-7xl mx-auto px-6">
          <div className="flex flex-wrap gap-4">
            {["All", "Social Media", "Paid Ads", "Content Creation", "Brand Strategy", "Case Studies"].map((cat, i) => (
              <button
                key={i}
        ...
        <section className="mt-16 max-w-7xl mx-auto px-6">
          <div className="grid md:grid-cols-3 gap-10">
            {blogPosts.map((post, index) => (
              <div key={index} className="bg-white rounded-3xl overflow-hidden shadow hover:shadow-xl transition">

                {/* Image */}
                <div className="relative h-48 w-full overflow-hidden">
                  {!loaded[index] && <div className="absolute inset-0 bg-gray-200 animate-pulse z-10" />}
                  ...
                  <img
                    ...
                    className={`relative z-20 h-full w-full object-cover transition-all duration-700
                      ${loaded[index] ? "opacity-100 blur-0 scale-100" : "opacity-0 blur-lg scale-105"}
                    `}
                  />
                </div>
